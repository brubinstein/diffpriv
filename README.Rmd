...
The `diffpriv` package makes privacy-aware data science under differential 
privacy easy in R. Differentially-private mechanisms can release as responses 
to untrusted third parties, models fit on privacy-sensitive data. But due to 
the worst-case nature of the framework, mechanism development typically 
requires involved theoretical analysis. `diffpriv` offers a turn-key approach
to differential privacy by automating this process with sensitivity sampling.
...
Next load the `diffpriv` package (see how to get it below) and construct a 
...
To run `mech` on `X` we must first deteremine the sensitivity of `target` to
small changes to input dataset. We could either assume bounded data, or in this
case we use sensitivity sampling: repeated probing of `target` to estimate 
sensitivity. We must specify a distribution `distr` for the probe datasets.
...
* Virtual class `DPMech` for generic mechanisms that captures the non-private
  `target` and releases privatized responses from it. Current subclasses
    ...
    + `DPMechExponential`: the exponential mechanism for privately 
      optimizing over finite sets (not necessarily numeric).
* Class `DPParamsEps` and subclasses for encapsulating privacy parameters.
* `sensitivitySampler` method in `DPMech` subclasses can be used when exact
  bounds on the sensitivity of non-private `target` aren't available. It
  repeatedly probes `target` to estimate sensitivity. Running mechanisms with
  obtained sensitivites yield slightly weaked random differential privacy.

Read the package vignette for more.

## Citing the Package

The `diffpriv` package is open-source with permissive MIT License. Please 
acknowledge use of `diffpriv` by citing the paper:
...
* Exponential mechanism: Frank McSherry and Kunal Talwar. "Mechanism design
  via differential privacy." In the 48th Annual IEEE Symposium on Foundations 
  of Computer Science (FOCS'07), pp. 94-103. IEEE, 2007.
