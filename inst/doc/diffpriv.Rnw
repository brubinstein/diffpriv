...
Laplace, Gaussian and exponential mechanisms; (b) a recent sensitivity sampler
...
on $D$ with a mechanism. A \term{mechanism} $M: \cD \to \cR$ is a
...
close on neighboring pairs: an adversarial observer can not determine an
unknown record from $M$ responses, even with knowledge of the other $n-1$
records.

Privacy parameters $\epsilon$ ($\epsilon,\delta$) are encapsulated in \pck
class \code{DPParamsEps} (\code{DPParamsDel} respectively). As most mechanisms
operate at tunable privacy levels, virtual \code{DPMech} generic method
...
Gaussian~\citep{dwork2014algorithmic} and
exponential~\citep{mcsherry2007mechanism} mechanisms as \code{DPMech}
subclasses \code{DPMechLaplace}, \code{DPMechGaussian} and
\code{DPMechExponential}. An exponential example is given in the next section.
\code{DPMechLaplace} is appropriate for releasing numeric
vectors. The Laplace mechanism requires that \cB be numeric $\reals^d$ for some
...
$L_2$-sensitivity and weaker $(\epsilon,\delta)$-DP.
